    ...
    <div class="user-profile-area d-flex flex-wrap">
        ...
        <div class="user-info d-flex flex-column">
            ...
            <div class="user-info-edit-items">
                ...
                <p class="info">{{ optional(json_decode(auth()->user()->user_information))->phone ?? 'N/A' }}</p>
                </div>
                <div class="item">
                <p class="title">{{ get_phrase('Address') }}</p>
                <p class="info">
                {{ optional(json_decode(auth()->user()->user_information))->address ?? 'N/A' }}
                </p>
                </div>
            </div>
            </div>
        ...
        <div class="user-details-info">
            ...
            >
                ...
                <form action="{{ route('teacher.profile.update') }}" method="post" enctype="multipart/form-data">
                    ...
                    >
                        <option value="Male" {{ $gender === 'male' ? 'selected' : '' }}>{{ get_phrase('Male') }}</option>
                        <option value="Female" {{ $gender === 'female' ? 'selected' : '' }}>{{ get_phrase('Female') }}</option>
                    </select>
                    </div>
                    <div class="fpb-7">
                    ...
                    <input
                        ...
                        id="eInputPhone"
                        name="phone"
                        value="{{ optional(json_decode(auth()->user()->user_information))->phone ?? '' }}"
                    ...
                    <input
                        ...
                        id="eInputAddress"
                        name="address"
                       value="{{ optional(json_decode(auth()->user()->user_information))->address ?? '' }}"
                    ...
                    >
                    <input type="hidden" class="form-control" name="old_photo" value="value="{{ optional(json_decode(auth()->user()->user_information))->photo ?? '' }}""/>
